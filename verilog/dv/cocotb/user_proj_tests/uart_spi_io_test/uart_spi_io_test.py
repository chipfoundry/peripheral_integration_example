...
@cocotb.test()
@report_test
async def uart_spi_io_test(dut):
    caravelEnv = await test_configure(dut, timeout_cycles=50000)

    cocotb.log.info(f"[TEST] Start uart_spi_io_test")
    ...
    # Wait for all UART I/O tests to complete (GPIO 2-8)
    for uart in range(7):
        await caravelEnv.wait_mgmt_gpio(uart + 2)
        cocotb.log.info(f"[TEST] UART{uart} I/O test completed")
        ...
        if tx_value == 0:
            cocotb.log.error(f"[TEST] UART{uart} TX pin {tx_pin} is not active")
        else:
            cocotb.log.info(f"[TEST] UART{uart} TX pin {tx_pin} is active")
    
    # Wait for all SPI I/O tests to complete (GPIO 9-14)
    for spi in range(6):
        await caravelEnv.wait_mgmt_gpio(spi + 9)
        cocotb.log.info(f"[TEST] SPI{spi} I/O test completed")
        
        # Verify SPI output pins are active
        ...
        if mosi_oeb != 0 or sclk_oeb != 0 or cs_oeb != 0:
            cocotb.log.error(f"[TEST] SPI{spi} output pins not properly configured")
        else:
            cocotb.log.info(f"[TEST] SPI{spi} output pins properly configured")
    
    # Wait for final completion signal
    await caravelEnv.wait_mgmt_gpio(0)
    cocotb.log.info(f"[TEST] All UART/SPI I/O tests completed successfully") 